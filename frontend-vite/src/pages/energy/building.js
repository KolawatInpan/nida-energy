...
export default function Building() {
                ...
                if (buildings && buildings.length) {
                    ...
                    if (found) {
                        data = {
                            ...
                            tradeMode: String(found.tradeMode || 'MANUAL').toUpperCase(),
                            tradeMeterType: String(found.tradeMeterType || 'produce').toLowerCase(),
                            batterySellThreshold: Number(found.batterySellThreshold ?? 80),
                            serviceUnitsList: []
                        };
                        ...
                        try {
                            ...
                            const normalizedMode = String(found.tradeMode || 'MANUAL').toUpperCase();
                            const persistedMeterType = String(found.tradeMeterType || 'produce').toLowerCase();
                            setTradeMode(normalizedMode);
                            setTradeMeterType(
                                normalizedMode === 'AUTO_BATTERY_THRESHOLD'
                                    ? 'battery'
                                    : normalizedMode === 'SELF_CONSUME'
                                        ? 'produce'
                                        : (persistedMeterType === 'consume' || persistedMeterType === 'battery' ? persistedMeterType : 'produce')
                            );
    ...
    const saveTradeSettings = async (nextMode, nextMeterType, nextThreshold) => {
        if (!building?.dbId) return;

        const normalizedMode = String(nextMode || 'MANUAL').toUpperCase();
        const normalizedMeterType = String(nextMeterType || 'produce').toLowerCase();
        const threshold = Number(nextThreshold);

        if (normalizedMode === 'AUTO_BATTERY_THRESHOLD' && (!Number.isFinite(threshold) || threshold < 0 || threshold > 100)) {
            alert('Auto Sell level must be between 0 and 100%.');
            return;
        }

        setIsSavingTrade(true);
        try {
            await updateBuilding(building.dbId, {
                tradeMode: normalizedMode,
                tradeMeterType: normalizedMeterType,
                batterySellThreshold: threshold,
            });
            alert('Trade settings saved successfully');
        } catch (error) {
            console.error('Failed to save trade settings:', error);
            alert('Failed to save trade settings: ' + (error?.response?.data?.error || error?.message || 'Unknown error'));
        } finally {
    ...
    const handleTradeModeChange = (newMode) => {
        const normalizedMode = String(newMode || 'MANUAL').toUpperCase();
        setTradeMode(normalizedMode);

        if (normalizedMode === 'AUTO_BATTERY_THRESHOLD') {
            setTradeMeterType('battery');
        } else if (normalizedMode === 'SELF_CONSUME') {
            setTradeMeterType('produce');
        }
    };

    const handleSaveTradeSettings = async () => {
        const enforcedMeterType = tradeMode === 'AUTO_BATTERY_THRESHOLD'
            ? 'battery'
            : tradeMode === 'SELF_CONSUME'
                ? 'produce'
                : tradeMeterType;

        await saveTradeSettings(tradeMode, enforcedMeterType, batterySellThreshold);
    };
