...
export default function EnergySelling() {
    ...
    const isManualMode = String(tradeMode || '').toUpperCase() === 'MANUAL';
    ...
    // Only sync tradeMode from selectedBuilding when opening a new building
    // Remove tradeMode sync from useEffect to prevent dropdown lock
    // Only set tradeMode in openBuildingPanel when opening a new building
    useEffect(() => {
      if (!selectedBuilding || !selectedBuilding.id) return;
      setUnsavedTradeMode(false);
    }, [selectedBuilding?.id]);

    // When user changes tradeMode, mark as unsaved
    const handleTradeModeChange = (value) => {
      setTradeMode(value);
      setUnsavedTradeMode(true);
    };
      ...
      {showPanel && selectedBuilding && (
        <div className="right-div">
          <EnergySellingPanel
            ...
            unsavedTradeMode={unsavedTradeMode}
            isSavingTradePolicy={isSavingTradePolicy}
