...
export default function MockEnergy() {
  ...
  const handleGenerate = async () => {
    ...
    // ensure both start and end are at local midnight
    s.setHours(0, 0, 0, 0);
    e.setHours(0, 0, 0, 0);
    ...
    try {
      // build generator per selected profile
      const createGenerator = (profileName) => {
      ...
      const interval = Math.max(1, parseInt(intervalHours) || 1);
      const generator = createGenerator(profile);

      // compute totals for progress
      const msPerHour = 60 * 60 * 1000;
      const hours = Math.ceil((e - s) / (interval * msPerHour));
      const total = selectedMeters.length * Math.max(0, hours);
      setTotalInserts(total);
      setCompletedInserts(0);
      setProgress(0);

      for (const snid of selectedMeters) {
        if (stopRequestedRef.current) {
          setStatus('Generation stopped');
          break;
        }

        setStatus(`Generating for ${snid}...`);
        ...
        while (cursor < e) {
          ...
          if (count === 1 || count % 12 === 0) {
            setStatus(`Generating ${snid}: ${count}/${hours}`);
          }

          cursor = new Date(cursor.getTime() + interval * msPerHour);
        }
        ...
        setCompletedInserts(prev => {
          const next = prev + logs.length;
          setProgress(total > 0 ? Math.round((next / total) * 100) : 100);
          return next;
        });
      }
      ...
      setStatus('Generation failed — see console');
    } finally {
      setSubmitting(false);
      setStopRequested(false);
      stopRequestedRef.current = false;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 md:p-8 bg-gradient-to-br from-sky-50 via-cyan-50 to-emerald-50 rounded-2xl border border-cyan-100 shadow-lg">
      ...
      <div className="flex flex-wrap gap-3 items-center bg-white/80 backdrop-blur rounded-xl border border-cyan-100 p-4 shadow-sm">
        ...
        <button onClick={handleStopGenerate} disabled={!submitting || stopRequested} className="px-4 py-2 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-lg shadow hover:from-amber-400 hover:to-orange-500 transition-colors disabled:opacity-60">
